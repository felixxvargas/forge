...
interface AppDataContextType {
  ...
  createPost: (content: string, images?: string[], url?: string, imageAlts?: string[], communityId?: string, gameId?: string, gameTitle?: string, gameIds?: string[], gameTitles?: string[], flareId?: string, commentsDisabled?: boolean, repostsDisabled?: boolean, replyTo?: string, quotePostId?: string, attachedList?: object, poll?: object, insightId?: string) => Promise<string | undefined>;
...
export function AppDataProvider({ children }: { children: ReactNode }) {
  ...
  const createPost = async (content: string, images?: string[], url?: string, imageAlts?: string[], communityId?: string, gameId?: string, gameTitle?: string, gameIds?: string[], gameTitles?: string[], flareId?: string, commentsDisabled?: boolean, repostsDisabled?: boolean, replyTo?: string, quotePostId?: string, attachedList?: object, poll?: object, insightId?: string): Promise<string | undefined> => {
    if (!session?.user) return undefined;
    const post = await postsAPI.create(session.user.id, content, { images, url, imageAlts, communityId, gameId, gameTitle, gameIds, gameTitles, flareId, commentsDisabled, repostsDisabled, replyTo, quotePostId, attachedList, poll, insightId });
    if (!replyTo) {
  ...
  const updateGameList = async (listType: GameListType, games: any[]) => {
    ...
    const newHiddenLists = wasEmpty && games.length > 0
      ...
      : hiddenLists;
    const prevCount = currentUserRef.current?.game_lists?._updateCount ?? existing._updateCount ?? 0;
    const updatedLists = { ...existing, [key]: games, hiddenLists: newHiddenLists, _updateCount: prevCount + 1 };
