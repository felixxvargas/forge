...
// Inline (non-fixed) version for Settings and Profile on mobile
export function OnboardingProgress() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .eq('user_id', currentUser.id)
      .limit(1)
      .then(({ count }) => setHasPosted((count ?? 0) > 0), () => setHasPosted(false));
  }, [currentUser?.id]);
  ...
  useEffect(() => {
    ...
    const stored = getStored();
    if (stored.dismissCount >= 3 && stored.forceServed) return;
    if (!stored.forceServed) saveStored({ ...stored, forceServed: true });
    setVisible(true);
  }, [isAuthenticated, currentUser?.id]);
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 px-3 py-2.5">
        <div className="flex gap-1 flex-1">
          {tasks.map((_, i) => (
            <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i < completedCount ? 'bg-accent' : 'bg-muted/60'}`} />
          ))}
        </div>
        <span className="text-xs text-muted-foreground whitespace-nowrap shrink-0">{completedCount}/{tasks.length}</span>
        <button onClick={() => setCollapsed(c => !c)} className="p-0.5 text-muted-foreground hover:text-foreground transition-colors rounded shrink-0" aria-label={collapsed ? 'Expand' : 'Collapse'}>
          <ChevronDown className={`w-4 h-4 transition-transform ${collapsed ? '' : 'rotate-180'}`} />
        </button>
        <button onClick={dismiss} className="p-0.5 text-muted-foreground hover:text-foreground transition-colors rounded shrink-0" aria-label="Dismiss">
          <X className="w-4 h-4" />
        </button>
      </div>
      {!collapsed && (
