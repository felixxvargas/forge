...
export function LoginModule({ variant = 'page', onSuccess }: Props) {
  ...
  return (
    <div className={isSidebar ? '' : 'min-h-screen flex flex-col items-center justify-center px-4 bg-background'}>
      <div className={isSidebar ? '' : 'w-full max-w-md flex-1 flex flex-col justify-center'}>
        {/* Branding */}
        <div className={`text-center ${isSidebar ? 'mb-5' : 'mb-8'}`}>
          <div className={`${isSidebar ? 'w-12 h-10' : 'w-20 h-20'} mx-auto ${isSidebar ? 'mb-2' : 'mb-3'} flex items-center justify-center`}>
            <svg width={isSidebar ? 48 : 80} height={isSidebar ? 38 : 64} viewBox="0 0 30 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <h1 className={`font-black tracking-tight text-accent ${isSidebar ? 'text-2xl mb-0.5' : 'text-4xl mb-1'}`}>Forge</h1>
          <p className={`text-muted-foreground ${isSidebar ? 'text-xs' : ''}`}>Your gaming social network</p>
        </div>

        {/* Card */}
        <div className={`bg-card rounded-2xl ${isSidebar ? 'p-5' : 'p-8'} shadow-xl`}>
          {!isSidebar && <h2 className="text-xl font-semibold mb-6 text-center">Sign In</h2>}
        ...
        {/* App Store Badges — only in page variant */}
        {!isSidebar && (
