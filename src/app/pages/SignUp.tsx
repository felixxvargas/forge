...
export function SignUp() {
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    if (!passwordValid) { setError('Please fix the password issues below.'); return; }
    if (!passwordsMatch) { setError('Passwords do not match.'); return; }
    if (HCAPTCHA_SITE_KEY && !captchaToken) { setShowCaptcha(true); return; }
          ...
          <div>
            ...
            <div className="relative">
              <input id="password" type={showPassword ? 'text' : 'password'} value={password}
                onChange={e => setPassword(e.target.value)} placeholder="Create a strong password" required disabled={isLoading}
          ...
          <div>
            ...
            <div className="relative">
              <input id="confirmPassword" type={showConfirm ? 'text' : 'password'} value={confirmPassword}
                onChange={e => setConfirmPassword(e.target.value)} placeholder="Repeat your password" required disabled={isLoading}
                className={`w-full px-4 py-3 pr-11 bg-white/[0.07] rounded-lg border focus:outline-none focus:ring-2 focus:ring-accent ${
                  confirmPassword.length > 0 ? (passwordsMatch ? 'border-accent/60' : 'border-destructive') : 'border-white/[0.12]'
                }`} />
      ...
      {showCaptcha && HCAPTCHA_SITE_KEY && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          ...
          <div className="relative w-full max-w-sm mx-4 bg-card rounded-t-2xl sm:rounded-2xl p-6 pb-10 sm:pb-6 flex flex-col items-center gap-4 shadow-2xl animate-in slide-in-from-bottom-4 sm:zoom-in-95 duration-200">
            ...
            <p className="text-sm text-muted-foreground text-center">Complete the challenge to create your account.</p>
            <HCaptcha ref={captchaRef} sitekey={HCAPTCHA_SITE_KEY}
              onVerify={(token) => { setCaptchaToken(token); setShowCaptcha(false); doSignUp(token); }}
