...
export function Explore() {
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      <div className={`sticky z-10 transition-all duration-300 border-b border-border bg-card/80 backdrop-blur-lg ${hideSearchBar ? 'top-14' : 'top-[130px]'} ${isSearchActive && activeTab !== 'games' ? 'hidden' : ''}`}>
                ...
                {(isSearchActive ? searchLoading : showGamesSkeleton) ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                    {(isSearchActive ? searchGames : filteredGames).map(game => {
                      ...
                      return (
                        <div key={game.id} className="group cursor-pointer relative hover:z-10 hover:bg-secondary/40 rounded-lg transition-colors duration-200" onClick={() => navigate(`/game/${game.id}`)}>
