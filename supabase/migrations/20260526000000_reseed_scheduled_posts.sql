-- Re-seed scheduled posts using a handle lookup so we don't rely on a
-- hardcoded user_id that may not exist in the auth.users table.
-- Prefers @forge; falls back to @felix.
...
BEGIN
  ...
  -- Remove any stale pending posts left over from previous seed attempts.
  DELETE FROM scheduled_posts WHERE status = 'pending';

  -- Release notes: Tuesdays at 10AM PDT (17:00 UTC), v0.3.0 → v0.3.7
  ...
  -- Blog posts: Mondays at 1PM PDT (20:00 UTC), every 2 weeks starting June 1
  INSERT INTO scheduled_posts (user_id, content, scheduled_at, status, game_ids, game_titles, images, url)
  VALUES
    (post_user_id,
